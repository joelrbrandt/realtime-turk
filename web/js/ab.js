...
var RANDOM_TASK_URL = "rts/ab/random"
 ...
 */
function abDataCallback(data) {
    $('#voteContainer').html("<table id='voteTable'><tr><td><div class='question'>" + data.question + "</div></td></tr><tr id='voteRow'></tr></table>")
    
    for (var i=0; i<data.photos.length; i++) {
        var newImg = $("<img src='" + data.photos[i].option + "' class='questionPic' />").data('pk', data.photos[i].pk);
        var row = $("<tr></tr>")
        var cell = $("<td></td>")
        cell.append(newImg);
        row.append(cell);
        $('#voteTable').append(row);
    }
...
function beginTask() {
    if (assignmentid != 0) {
        ...
        logEvent("accept", null, function() {
            scheduleRetainer(abDataCallback, RANDOM_TASK_URL);
            retainerHide();
