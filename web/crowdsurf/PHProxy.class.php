...
class PHProxy
{
  ...
  // iphone
  // var $THE_USER_AGENT = "Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_0 like Mac OS X; en-us) AppleWebKit/532.9 (KHTML, like Gecko) Version/4.0.5 Mobile/8A293 Safari/6531.22.7";

  // winmo 6.0
  var $THE_USER_AGENT = "Mozilla/4.0 (compatible; MSIE 6.0; Windows CE; IEMobile 6.12)";
  ...
  //
  // Edit the $config variables in index.php and poxy_javascript.js instead
  //
  ...
  function modify_urls()
  {
	...
	//error_log("phproxy: processing $tag", 0);
	//error_log("phproxy:   matches[1] = {$matches[1][$i]}", 0);
	//error_log("phproxy:   matches[2] = {$matches[2][$i]}", 0);
	...
	//error_log("phproxy: still processing $tag", 0);

	preg_match_all("#([a-zA-Z\-\/]+)\s*(?:=\s*(?:\"([^\">]*)\"?|'([^'>]*)'?|([^'\"\s]*)))?#", $matches[2][$i], $m, PREG_SET_ORDER);
	...
	ob_start();
	var_dump($attrs);
	$log_msg = "phproxy: " . ob_get_contents();
	//error_log($log_msg);
	ob_end_clean();
	...
	switch ($tag)
	  ...
	  case 'body':
	    //$rebuild = true;
	    //$extra_html .= "\n<div style='z-index: 999999; position: fixed; top: 10px; right: 10px; width=300px; background-color: #666666'>I'm in ur codez</div>\n";
  ...
  function set_request_headers()
  {
    ...
    if (isset($this->THE_USER_AGENT)) {
      $headers .= 'User-Agent: ' . $this->THE_USER_AGENT . "\r\n";
      //error_log("phproxy: setting custom user agent", 0);
      
    }
    else if (isset($_SERVER['HTTP_USER_AGENT']))
      {
	$headers .= 'User-Agent: ' . $_SERVER['HTTP_USER_AGENT'] . "\r\n";
	//error_log("phproxy: setting boring user agent", 0);
      }
