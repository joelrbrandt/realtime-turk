...
def log(request):
    ...
    try:    
        ...
        if event == "accept":
            db.query_and_return_array("""INSERT INTO assignments (assignmentid, workerid, hitid, accept) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE workerid = %s, `accept` = %s, `show` = NULL, `go` = NULL, `first` = NULL, `submit` = NULL, `moleid` = NULL""", (assignment, worker, hit, time, worker, time))
    except:
