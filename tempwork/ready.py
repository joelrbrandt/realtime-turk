...
"""Returns information about the job to do, if one is ready. Otherwise, tells the worker to continue waiting."""
def is_ready(request):
    ...
    ### WE LEFT OFF HERE
    # Better architecture, next step: turn this into a single call:
    # getPendingJob(): returns None if there is no work to do
    #                  returns a json object with at least the field 'jobid', and any other info you want passed back to the Javascript


    # Find out if there are any jobs that are pending
###    result = getVotesNeedingOpinions(db, workerid)
    results = getPendingJobs(db, workerid) # TODO: getPendingJobs
    ...
    if len(result) == 0:
        ...
        job = result[0]
        result = getAndAssignJob(job, assignmentid, db)
        request.write(json.dumps(result, cls=DecimalEncoder))
    
def getAndAssignJob(job, assignmentid, db):
    jobid = job['pk']
    updateAssignment(assignmentid, jobid)
    
    logging.debug("Job needing workers: " + str(jobid))
